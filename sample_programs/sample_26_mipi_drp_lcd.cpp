...
#define CAMERA_PIXEL_HW        (1280u)
#define CAMERA_PIXEL_VW        (720u)

#define CAMERA_BUFFER_STRIDE   (((CAMERA_PIXEL_HW * 1) + 31u) & ~31u)
...
static uint8_t fbuf_camera[CAMERA_BUFFER_STRIDE * CAMERA_BUFFER_HEIGHT]__attribute((aligned(128)));
...
static void drp_task(void) {
    ...
    while (true) {
        ...
        for (uint32_t y = 0; y < VIDEO_PIXEL_VW; y++) {
            memcpy((void *)&fbuf_bayer[VIDEO_PIXEL_HW * y], (void *)&fbuf_camera[CAMERA_PIXEL_HW * y], FRAME_BUFFER_STRIDE);
        }
